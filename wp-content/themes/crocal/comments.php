...
<?php if ( have_comments() ) : ?>
	...
	<div id="eut-comments" class="eut-container eut-padding-top-3x eut-padding-bottom-2x eut-border">
		<div class="eut-comments-header eut-margin-bottom-1x eut-align-center">
			<div class="eut-comments-number eut-h5">
			<?php comments_number(); ?>
			</div>
			<nav class="eut-comment-nav eut-link-text eut-heading-color" aria-label="<?php esc_attr_e( 'Comments', 'crocal' ); ?>">
				<ul>
...
<?php
		...
		$args = array(
			...
			'comment_field' =>
				...
				'<textarea id="comment" name="comment" placeholder="' . esc_attr__( 'Your Comment Here...', 'crocal' ) . '" aria-label="' . esc_attr__( 'Your Comment Here...', 'crocal' ) . '" cols="45" rows="15" aria-required="true">' .
				'</textarea></div>',
			...
			'fields' => apply_filters(
				...
				array(
					'author' =>
						...
						' placeholder="' . esc_attr__( 'Name', 'crocal' ) . ' ' . ( $req ? esc_attr__( '(required)', 'crocal' ) : '' ) . '" aria-label="' . esc_attr__( 'Name', 'crocal' )  . '" />' .
						'</div>',

					'email' =>
						'<div class="eut-form-input eut-half-size eut-border">' .
						'<input id="email" name="email" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '"' .
						' placeholder="' . esc_attr__( 'E-mail', 'crocal' ) . ' ' . ( $req ? esc_attr__( '(required)', 'crocal' ) : '' ) . '" aria-label="' . esc_attr__( 'E-mail', 'crocal' )  . '" />' .
						'</div>',

					'url' =>
						'<div class="eut-form-input eut-border">' .
						'<input id="url" name="url" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) . '"' .
						' placeholder="' . esc_attr__( 'Website', 'crocal' )  . '" aria-label="' . esc_attr__( 'Website', 'crocal' )  . '" />' .
						'</div>',
					)
				),
		);
