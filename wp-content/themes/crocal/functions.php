...
define( 'CROCAL_EUTF_THEME_VERSION', '1.4.1' );
...
if ( ! function_exists( 'crocal_eutf_modal_wpsearch' ) ) {
	function crocal_eutf_modal_wpsearch() {
		...
		if( !empty( $search_modal_text ) ) {
			$form .= '<label class="eut-search-title eut-heading-color eut-h2" for="eut-search-modal-textfield">' . wp_kses( $search_modal_text , array( 'br' => array() ) ) . '</label>';
		}
		$form .= '  <input type="text" id="eut-search-modal-textfield" class="eut-search-textfield eut-h2 eut-border" value="' . get_search_query() . '" name="s" autocomplete="off"/>';
...
function crocal_eutf_frontend_scripts() {
	...
	wp_enqueue_style( 'font-awesome', get_template_directory_uri() . '/css/all.min.css', array(), '5.11.2' );
	wp_enqueue_style( 'font-awesome-v4-shims', get_template_directory_uri() . '/css/v4-shims.min.css', array( 'font-awesome' ), '5.11.2' );
	...
	$crocal_eutf_main_data = array(
		...
		'back_to_top' => crocal_eutf_option( 'back_to_top_enabled', '1' ),
		'string_go_back' => esc_html__( 'Back', 'crocal' ),
	...
	$crocal_eutf_data = array(
		'quantity_buttons' => crocal_eutf_option( 'product_woo_quantity_buttons', '1' ),
	);
	...
	if ( '1' == crocal_eutf_option( 'combine_js', '1' ) ) {
		...
		wp_localize_script( 'crocal-eutf-extras', 'crocal_eutf_data', $crocal_eutf_data );
		...
		wp_localize_script( 'crocal-eutf-eut', 'crocal_eutf_data', $crocal_eutf_data );
		wp_localize_script( 'crocal-eutf-main-script', 'crocal_eutf_main_data', $crocal_eutf_main_data );
		if ( function_exists( 'wp_add_inline_script' ) ) {
			wp_add_inline_script( 'crocal-eutf-main-script', $resolution_code );
			wp_add_inline_script( 'crocal-eutf-main-script', crocal_eutf_get_privacy_cookie_script() );
...
function crocal_eutf_vc_frontend_css() {
	...
	if ( wp_style_is( 'font-awesome', 'registered' ) ) {
		...
		wp_register_style( 'font-awesome', get_template_directory_uri() . '/css/all.min.css', array(), '5.11.2' );
	}

}
add_action( 'vc_base_register_front_css', 'crocal_eutf_vc_frontend_css' );
add_action( 'elementor/frontend/after_register_styles', 'crocal_eutf_vc_frontend_css' );
