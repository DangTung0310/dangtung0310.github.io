...
if (!class_exists("Crocal_Eutf_Redux_Framework_config")) {
	...
	class Crocal_Eutf_Redux_Framework_config {
		...
		public function setSections() {
			...
			$privacy_initial_selector = array(
				'gtracking'     => 'Google Tracking',
				'gfonts' => 'Google Fonts',
				'gmaps' => 'Google Maps',
				'video-embeds'   => 'Embed Videos',
			);
			$crocal_eutf_options = get_option( 'crocal_eutf_options' );
			if ( isset( $crocal_eutf_options['privacy_custom_fields'] ) ) {
				$privacy_custom_fields = $crocal_eutf_options['privacy_custom_fields'];
				if( !empty( $privacy_custom_fields ) ) {
					foreach( $privacy_custom_fields as $key => $value ) {
						if( $value ) {
							$id = sanitize_title_with_dashes( $value );
							$privacy_initial_selector[$id] = $id;
						}
					}
				}
			}
			...
			$regenerate_link = '<a href="//wordpress.org/plugins/regenerate-thumbnails/" target="_blank" rel="noopener noreferrer">' . esc_html__( 'regenerate your thumbnails', 'crocal' ) . '</a>';
			$crocal_gmap_api_key_link = '<a href="//developers.google.com/maps/documentation/javascript/get-api-key" target="_blank" rel="noopener noreferrer">' . esc_html__( 'Generate Google Map API Key', 'crocal' ) . '</a>';
			$crocal_gmap_style_link = '<a href="//mapstyle.withgoogle.com/" target="_blank" rel="noopener noreferrer">' . esc_html__( 'Styling Wizard Google Maps API.', 'crocal' ) . '</a>';
			...
			$this->sections[] = array(
				...
				'fields' => array(
					...
					array(
						...
						'default' => '<p>With <i class="eut-text-primary-1 fas fa-heart"></i> by <a href="//themeforest.net/user/euthemians/portfolio" target="_blank" rel="noopener noreferrer">Euthemians</a> - Premium Themes</p>',
						'required' => array(
			...
			$this->sections[] = array(
				...
				'fields' => array(
					...
					array(
						...
						'type' => 'info',
						'class' => 'eut-redux-sub-info',
						'title' => esc_html__( 'Shop Extras', 'crocal' ),
						'subtitle'=> esc_html__( 'Set the style for additional shop settings.', 'crocal' ),
					),
					array(
						'id'=>'product_woo_quantity_buttons',
						'type' => 'switch',
						'title' => esc_html__( 'Product Quantity Buttons', 'crocal' ),
						'subtitle'=> esc_html__( 'Enable or Disable -/+ quantity buttons.', 'crocal' ),
						"default" => '1',
					),
				)
			);
			...
			$this->sections[] = array(
				...
				'fields' => array(
					array(
						'id'=>'social_options_link_target',
						'type' => 'select',
						'title' => esc_html__( 'Social URLs Link Target', 'crocal' ),
						'subtitle'=> esc_html__( 'Define the target for your social URLs, same or new page.', 'crocal' ),
						'options' => array(
			...
			$this->sections[] = array(
				...
				'fields' => array(
					...
					array(
						...
						'desc' => sprintf( '%1$s: <a href="//wiki.openstreetmap.org/wiki/Tile_servers" target="_blank" rel="noopener noreferrer"> %2$s </a>', esc_html__('See more tile servers', 'crocal'), esc_html__('here', 'crocal') ),
			...
			$this->sections[] = array(
				...
				'fields' => array(
					...
					array(
						...
						'options'  => $privacy_initial_selector,
						'default' => array(
							'gtracking'     => '0',
							'gfonts' => '0',
							'gmaps' => '0',
