jQuery(document).ready(function($) {
	...
	$(document).on("click",".eut-upload-slider-button",function() {
		...
		eutMediaSliderFrame = wp.media.frames.eutMediaSliderFrame = wp.media({
			...
			frame: 'select',
			multiple: 'toggle',
			title: crocal_eutf_upload_slider_texts.modal_title,
			library: {
				type: 'image'
			},
			button: {
				text:  crocal_eutf_upload_slider_texts.modal_button_title
			}
		...
		eutMediaSliderFrame.on('select', function(){
			...
			var dataParams = {
				action:'crocal_eutf_get_slider_media',
				attachment_ids: ids.toString(),
				_eutf_nonce: crocal_eutf_upload_slider_texts.nonce_slider_media
			};
			$.post( crocal_eutf_upload_slider_texts.ajaxurl, dataParams, function( mediaHtml ) {
