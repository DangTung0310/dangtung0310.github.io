...
if ( !function_exists( 'crocal_eutf_print_breadcrumbs' ) ) {
	function crocal_eutf_print_breadcrumbs( $args = array() ) {
		//Yoast Breadcrumbs
		if ( function_exists( 'yoast_breadcrumb' ) ) {
			yoast_breadcrumb( '<nav aria-label="' . esc_attr( 'Breadcrumbs', 'crocal' ) . '"', '</nav>' );
...
<?php
			...
			$breadcrumb = new Crocal_Eutf_Breadcrumb( $args );
			return $breadcrumb->trail();
		}
	}
}

class Crocal_Eutf_Breadcrumb {
	...
	public function __construct( $args = array() ) {
		...
		$defaults = array(
			...
			'after'           => '',
			'show_on_front'   => true,
			'home_label'       => esc_html__( 'Home', 'crocal' ),
			'show_title'      => true,
			'post_taxonomy'   => array(
				'post' => 'category',
			),
		...
		$args = apply_filters( 'crocal_eutf_breadcrumb_args', $args );
	...
	public function trail() {
		...
		if ( 0 < $item_count ) {
			...
			$breadcrumb .= '<ul class="eut-breadcrumb-items" itemscope itemtype="http://schema.org/BreadcrumbList">';
			...
			foreach ( $this->items as $item ) {
				...
				$item_class = 'eut-breadcrumb-item eut-small-text';
				if ( 1 === $item_position && 1 < $item_count ) {
					$item_class .= ' eut-breadcrumb-first';
				} elseif ( $item_count === $item_position ) {
					$item_class .= ' eut-breadcrumb-last';
				}
			...
			echo sprintf(
				'<%1$s id="eut-breadcrumbs" class="eut-list-divider">%2$s%3$s%4$s</%1$s>',
	...
	protected function add_items() {
		...
		if ( is_front_page() ) {
			...
			if ( $home_label ) {
				$this->items[] = sprintf( '<a itemprop="item" href="%s"%s>%s</a>', esc_url( home_url( '/' ) ), ' rel="home"', $home_label );
			}
			...
			if ( is_home() ) {
				...
				if ( is_post_type_archive() ) {
					...
					$this->add_rewrite_front_items();
					$year  = sprintf( '%s',  get_the_time( esc_html__( 'Y',  'crocal' ) ) );
					$month = sprintf( '%s', get_the_time( esc_html__( 'F', 'crocal' ) ) );
					$day   = sprintf( '%s',   get_the_time( esc_html__( 'j', 'crocal' ) ) );
					...
					if ( true === $this->args['show_title'] ) {
						$this->items[] = esc_html__( 'Archives', 'crocal' );
					}
				}
			} elseif ( is_search() ) {
				//Search Page
				if ( is_paged() ) {
					$this->items[] = sprintf( '<a itemprop="item" href="%s">%s</a>', esc_url( get_search_link() ), sprintf( esc_html__( 'Search Results for : %s', 'movedo' ), get_search_query() ) );
				} elseif ( true === $this->args['show_title'] ) {
					$this->items[] = sprintf( esc_html__( 'Search Results for : %s', 'crocal' ), get_search_query() );
				}
			} elseif ( is_404() ) {
				//404 Page
				if ( true === $this->args['show_title'] ) {
					$this->items[] = esc_html__( '404 Not Found', 'crocal' );
				}
			}
		}
		...
		if ( is_singular() && 1 < get_query_var( 'page' ) && true === $this->args['show_title'] ) {
			$this->items[] = sprintf( esc_html__( 'Page %s', 'crocal' ), number_format_i18n( absint( get_query_var( 'page' ) ) ) );
		} elseif ( is_paged() && true === $this->args['show_title'] ) {
			$this->items[] = sprintf( esc_html__( 'Page %s', 'crocal' ), number_format_i18n( absint( get_query_var( 'paged' ) ) ) );
		}

		//Filter hook for items
		$this->items = array_unique( apply_filters( 'crocal_eutf_breadcrumb_items', $this->items, $this->args ) );
	}
