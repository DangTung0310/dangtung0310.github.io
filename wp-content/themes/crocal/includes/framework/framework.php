<?php
    ...
    //REDUX_MOD_TOM
        ...
        class ReduxFramework {
            ...
            public static function init() {
                ...
                if ( self::$_is_plugin == true || self::$_as_plugin == true ) {
                    ...
                    if ( strpos( Redux_Helpers::cleanFilePath( __FILE__ ), Redux_Helpers::cleanFilePath( get_template_directory() ) ) !== false ) {
                        ...
                        //REDUX_MOD_TOM
                        self::$_url   = trailingslashit( get_template_directory_uri() . '/includes/framework/' );
                    }
                }
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( empty ( $this->args['footer_credit'] ) ) {
                    $this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( __( 'Options panel created using %1$s', 'crocal' ), '<a href="' . esc_url( $this->framework_url ) . '" target="_blank">' . __( 'Redux Framework', 'crocal' ) . '</a> v' . self::$_version ) . '</span>';
                }

                if ( empty ( $this->args['menu_title'] ) ) {
                    $this->args['menu_title'] = __( 'Options', 'crocal' );
                }

                if ( empty ( $this->args['page_title'] ) ) {
                    $this->args['page_title'] = __( 'Options', 'crocal' );
                }
                ...
                if ( ! empty ( $this->args['opt_name'] ) ) {
                    ...
                    // Options page
					//REDUX_MOD_TOM
                    add_action( 'admin_menu', array( $this, '_options_page' ), 11 );
            ...
            private function _internationalization() {
                 ...
                 * @param string     'crocal'  text domain
                 */
                ...
                load_plugin_textdomain( 'crocal', false, $basepath . 'languages' );
            }
            ...
            public function _options_page() {
                ...
                if ( $this->args['menu_type'] == 'hidden' ) {
                    ...
                    add_submenu_page( 'crocal', $this->args['page_title'], $this->args['menu_title'], $this->args['page_permissions'], $this->args['page_slug'], array(
                            &$this,
                            'generate_panel'
                        ) );
                } else {
            ...
            public function _enqueue_output() {
                ...
                if ( ! empty ( $this->typography ) && ! empty ( $this->typography ) && filter_var( $this->args['output'], FILTER_VALIDATE_BOOLEAN ) ) {
                    ...
                    //REDUX_MOD_TOM
                    if ( ! $this->args['disable_google_fonts_link'] ) {
                        //REDUX_MOD_TOM
                        $protocol = is_ssl() ? "https:" : "http:";

                        wp_enqueue_style( 'redux-google-fonts-' . $this->args['opt_name'], $protocol . $typography->makeGoogleWebfontLink( $this->typography ), '', $version );
                    }
                }
            }
            // _enqueue_output()
        ...
        //REDUX_MOD_TOM
                require_once get_template_directory() . '/includes/framework/core/enqueue.php';
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    $event = __( 'moving the mouse over', 'crocal' );
                    if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {
                        $event = __( 'clicking', 'crocal' );
                    }

                    // Construct message
                    $msg = sprintf( __( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'crocal' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;

                    // Construct hint tab
                    $tab = array(
                        'id'      => 'redux-hint-tab',
                        'title'   => __( 'Hints', 'crocal' ),
                        'content' => '<p>' . $msg . '</p>'
                    );
            ...
            private function get_default_output_string( $field ) {
                ...
                if ( ! empty ( $default_output ) ) {
                    $default_output = __( 'Default', 'crocal' ) . ": " . substr( $default_output, 0, - 2 );
                }
            ...
            public function ajax_save() {
                if ( ! wp_verify_nonce( $_REQUEST['nonce'], "redux_ajax_nonce" . $this->args['opt_name'] ) ) {
                    echo json_encode( array(
                        'status' => __( 'Invalid security credential.  Please reload the page and try again.', 'crocal' ),
                        'action' => ''
                    ) );
                ...
                if ( ! current_user_can( $this->args['page_permissions'] ) ) {
                    echo json_encode( array(
                        'status' => __( 'Invalid user capability.  Please reload the page and try again.', 'crocal' ),
                        'action' => ''
                    ) );
                ...
                if ( ! empty ( $_POST['data'] ) && ! empty ( $redux->args['opt_name'] ) ) {
                    ...
                    if ( ! empty ( $values ) ) {
                        ...
                        echo json_encode( array( 'status' => __( 'Your panel has no fields. Nothing to save.', 'crocal' ) ) );
                    }
                }
                if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {
                ...
                if ( isset( $return_array ) ) {
                    if ( $return_array['status'] == "success" ) {
                        //REDUX_MOD_TOM
                        require_once get_template_directory() . '/includes/framework/core/panel.php';
            ...
            public function _field_input( $field, $v = null ) {
                ...
                if ( isset ( $field['type'] ) ) {
                    ...
                    if ( class_exists( $field_class ) ) {
                         ...
                         */
						 //REDUX_MOD_TOM
                        echo apply_filters( "redux/field/{$this->args['opt_name']}/render/after", $_render, $field );						
                            ...
                            if ( is_array( $arr ) && ! empty( $arr ) ) {
                                foreach ( $arr as $x => $y ) {
                                    if ( strpos( strtolower( $y ), 'redux' ) !== false ) {
                                        $msg = __( '<strong>Redux Framework Notice: </strong>There are references to the Redux Framework support site in your config\'s <code>admin_bar_links</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'crocal' );
                    ...
                    if ( ! empty( $this->args['share_icons'] ) ) {
                        foreach ( $this->args['share_icons'] as $idx => $arr ) {
                            if ( is_array( $arr ) && ! empty( $arr ) ) {
                                foreach ( $arr as $x => $y ) {
                                    if ( strpos( strtolower( $y ), 'redux' ) !== false ) {
                                        $msg = __( '<strong>Redux Framework Notice: </strong>There are references to the Redux Framework support site in your config\'s <code>share_icons</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'crocal' );
