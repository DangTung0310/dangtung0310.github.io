			...
			<div class="<?php echo esc_attr( $crocal_eutf_footer_bar_class_string ); ?>" data-align-center="<?php echo esc_attr( $align_center ); ?>">
				<div class="eut-container">
					...
					<div class="eut-bar-content eut-right-side">
						<nav class="eut-footer-menu" aria-label="<?php esc_attr_e( 'Footer', 'crocal' ); ?>">
						...
						if ( !empty( $options ) && !empty( $social_options ) ) {
							if ( 'text' == $social_display ) {
									...
									if ( isset( $options[$key] ) && 1 == $options[$key] && $value ) {
										if ( 'skype' == $key ) {
											...
											if ( '_blank' == $link_target ) {
												echo '<li><a href="' . esc_url( $value ) . '" target="_blank" rel="noopener noreferrer">' . esc_html( $crocal_eutf_social_list_extended[$key]['title'] ) . '</a></li>';
											} else {
												echo '<li><a href="' . esc_url( $value ) . '">' . esc_html( $crocal_eutf_social_list_extended[$key]['title'] ) . '</a></li>';
											}
										}
									}
								}
								...
								foreach ( $social_options as $key => $value ) {
									if ( isset( $options[$key] ) && 1 == $options[$key] && $value ) {
										if ( 'skype' == $key ) {
											echo '<li><a href="' . esc_url( $value, array( 'skype', 'http', 'https' ) ) . '" class="' . esc_attr( $crocal_eutf_social_list_extended[$key]['class'] ) . '"><span class="screen-reader-text">' . esc_html( $crocal_eutf_social_list_extended[$key]['title'] ) . '</span></a></li>';
										} else {
											$link_target = crocal_eutf_option('social_options_link_target', '_blank');
											if ( '_blank' == $link_target ) {
												echo '<li><a href="' . esc_url( $value ) . '" target="_blank" rel="noopener noreferrer" class="' . esc_attr( $crocal_eutf_social_list_extended[$key]['class'] ) . '"><span class="screen-reader-text">' . esc_html( $crocal_eutf_social_list_extended[$key]['title'] ) . '</span></a></li>';
											} else {
												echo '<li><a href="' . esc_url( $value ) . '" class="' . esc_attr( $crocal_eutf_social_list_extended[$key]['class'] ) . '"><span class="screen-reader-text">' . esc_html( $crocal_eutf_social_list_extended[$key]['title'] ) . '</span></a></li>';
											}
										}
									}
								}
		...
		?>
			<div class="eut-back-top" data-speed="<?php echo esc_attr( $crocal_eutf_back_to_top_speed ); ?>">
				<div class="<?php echo esc_attr( $crocal_eutf_back_to_top_icon_wrapper_class_string ); ?>">
					<svg class="eut-back-top-icon" width="18px" height="18px" viewBox="0 0 18 18" aria-labelledby="eut-btt-title" role="img"><title id="eut-btt-title"><?php esc_html_e( 'Back to top', 'crocal' ); ?></title><g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd"><polygon fill="#000000" fill-rule="nonzero" points="10 3.75735931 10 18 8 18 8 3.89949494 3.41421356 8.48528137 2 7.07106781 9.07106781 0 16.1421356 7.07106781 14.7279221 8.48528137"></polygon></g></svg>
				</div>
			</div>
		<?php
		}
