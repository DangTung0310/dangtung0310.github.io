...
if (! function_exists('sc_example_component')) {
	function sc_example_component($attr, $content = null)
	{
		extract(shortcode_atts(array(
			'title' => ''
		), $attr));

		$output = '<div class="example-component">';
		if ($title) {
			$output .= '<div class="example-component__title">' . $title . '</div>';
		}
		$output .= '</div>';
		$output .= "\n";
